...
int main(int argc, char *argv[]) {
  ...
  double pSize = n/mpi_p;
  double *pBlock = malloc(sizeof(double)*pSize);
  if (mpi_rank == 0) {
  ...
  MPI_Gather(pBlock, pSize, MPI_DOUBLE, v, pSize, MPI_DOUBLE, 0,
           MPI_COMM_WORLD);
           	double endOne = MPI_Wtime();
	merge_sort(n,v);
 	double endTwo = MPI_Wtime();

	if (mpi_rank == 0) {
		printf("Total time to solve with %d MPI Processes was %.6f\n Time to Solve Scatter and Gather %.6f\n",
				mpi_p, (endTwo-start),(endOne-start));
