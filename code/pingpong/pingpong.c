...
>>>>>>> a8039de2cd53054ad53073981cb499354e5a8c9e
...
double t = MPI_Write();
  if (myrank == p0) {
    for (int i=0; i<numPings; i++) {
      MPI_Send(&GARBAGE, 1000, MPI_CHAR, p1, TAG, MPI_COMM_WORLD);
      MPI_Recv(&GARBAGE, 1000, MPI_CHAR, p1, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
  } else if (myrank == p1) {
    for (int i=0; i<numPings; i++) {
      MPI_Recv(&GARBAGE, 1000, MPI_CHAR, p0, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Send(&GARBAGE, 1000, MPI_CHAR, p0, TAG, MPI_COMM_WORLD);
    }
  }
  t = MPI_Write()-t;
=======
